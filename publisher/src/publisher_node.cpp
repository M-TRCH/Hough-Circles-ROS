...
void imageCallback(const sensor_msgs::ImageConstPtr& src_img)
{
  ...
  */
  ...
  cv::HoughCircles(src_out, circles, CV_HOUGH_GRADIENT, 1, 150, 200, 100, 20, 1500);

  for(int i = 0; i < circles.rows; i++)
  {
    cv::Point center(cvRound(circles(i)(0)), cvRound(circles(i)(1)));
    int radius = cvRound(circles(i)(2));
    cv::circle(src_in, center, radius, CV_RGB(255, 0, 0), 3);
  }
...
{
  ros::init(argc, argv, "circle_detected");
  ros::NodeHandle n;
